...
def calculaConcentracao(comprimento, numeroVolumes, t_inicial, t_final, velocidade, alpha, ca, cb, cc):
    ...
    for n in range(len(tempo)):
        ...
        for i in range(numeroVolumes):
            ...
            if i == 0:
                #Q[i-1] = 2 * ca - Q[i]
                q_n = Q[i] - (velocidade*DeltaT / DeltaX)*(Q[i+1] - (2 * ca - Q[i])) + (velocidade**2 * DeltaT**2 / (2*DeltaX**2))*(Q[i+1]-2*Q[i] + 2 * ca - Q[i])
            elif i == numeroVolumes - 1:
                #Q[i+1] = Q[i]
                q_n = Q[i] - (velocidade*DeltaT/DeltaX)*(Q[i]- Q[i-1]) + (velocidade**2 * DeltaT**2 / (2*DeltaX**2))*(Q[i] - 2*Q[i]+ Q[i-1])
            else:
                q_n = Q[i] - (velocidade*DeltaT/DeltaX)*(Q[i+1]-Q[i-1]) + (velocidade**2 * DeltaT**2 / (2*DeltaX**2))*(Q[i+1] -2*Q[i]+Q[i-1])
