...
def calculaConcentracao(comprimento, numeroVolumes, t_inicial, t_final, velocidade, alpha, ca, cb, cc):
    ...
    # DeltaT = 0.5/((velocidade/DeltaX)+(2*alpha/(DeltaX**2)))   
    DeltaT = 0.4* DeltaX/ (velocidade)   #estabilidade pro método
    ...
    for n in range(len(tempo)):
        ...
        for i in range(numeroVolumes):
            ...
            if i == 0:
                q_n = (Q[i+1]+ 2 *ca - Q[i])/2 -((velocidade*DeltaT)/DeltaX)* (Q[i+1] - (2 *ca - Q[i]))
            elif i == numeroVolumes - 1:
                q_n = (Q[i]+Q[i-1])/2 - ((velocidade*DeltaT)/DeltaX)*(Q[i]-Q[i-1])
            else:
                q_n = (Q[i+1]+Q[i-1])/2 - ((velocidade*DeltaT)/DeltaX)*(Q[i+1]-Q[i-1])
