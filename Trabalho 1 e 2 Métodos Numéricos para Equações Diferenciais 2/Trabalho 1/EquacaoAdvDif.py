...
def calculaConcentracao(comprimento, numeroVolumes, t_inicial, t_final, velocidade, alpha, ca, cb, cc):
    ...
    DeltaT = 0.9/((velocidade/DeltaX)+(2*alpha/(DeltaX**2)))   
        ...
        for i in range(numeroVolumes):
            ...
            if i == 0:
                q_n = Q[i] - (DeltaT / DeltaX) * (
                    velocidade * (Q[i] - (2 * ca - Q[i])) - alpha * (Q[i + 1] - 2 * Q[i] + (2 * ca - Q[i])) / (DeltaX)
                )
                ...
                q_n = Q[i] - (DeltaT / DeltaX) * (
                    velocidade * (Q[i] - Q[i - 1]) - alpha * (Q[i + 1] - 2 * Q[i] + Q[i - 1]) / (DeltaX)
                )
...
def Alphas(comprimento,  numeroVolumes, t_inicial, t_final, velocidade, alpha,ca, cb, cc):
    alphas = [0,alpha]
    for i in range(len(alphas)):
        volume, Solucao = calculaConcentracao(comprimento,  numeroVolumes, t_inicial, t_final, velocidade, alphas[i],ca, cb, cc)
        plt.plot(volume,Solucao, label=str(alphas[i]))
        plt.legend(title = r'$\alpha$', loc=0)
        
    plt.title(r'$Influência$ $do$ $\alpha$')
    plt.xlabel(r'$Comprimento$')
    plt.ylabel(r'$Concentração$')
    plt.show()
...
def SensibilidadeAlpha(comprimento,  numeroVolumes, t_inicial, t_final, velocidade, alphaInicial, alphaFinal,ca, cb, cc,qtdComparacoes):
    passoAlpha=float((alphaFinal-alphaInicial)/qtdComparacoes)
    alphas = np.arange(alphaInicial,alphaFinal, passoAlpha)
    for i in range(len(alphas)):
        volume, Solucao = calculaConcentracao(comprimento,  numeroVolumes, t_inicial, t_final, velocidade, alphas[i],ca, cb, cc)
        plt.plot(volume,Solucao, label=str("%.1f" %alphas[i]))
        plt.legend(title = r'$ \alpha$', loc=0)
        
    plt.title(r'$Sens. \alpha$')
    plt.xlabel(r'$Comprimento$')
    plt.ylabel(r'$Concentração$')
    plt.show()
...
def Sensibilidadecc(comprimento,  numeroVolumes, t_inicial, t_final, velocidade, alpha,ca, cb, ccInicial, ccFinal, qtdComparacoes):
    ...
    ccs = np.arange(ccInicial, ccFinal, passocc/2)
...
def SensibilidadeTempoFinal(comprimento,  numeroVolumes, t_inicial, t_final, velocidade, alpha,ca, cb, cc, qtdComparacoes):
    ...
    tempos = np.arange(t_inicial+passoTempo, t_final+passoTempo, passoTempo/2)
...
# PlotaGrafico(comprimento=50, numeroVolumes=500,t_inicial= 0,t_final=5,velocidade=1,alpha= 100*10**(-2),ca=0.8,cb= 0.1,cc= 1.0)
# Refinamento(comprimento=50,numeroVolumesInicial= 100,numeroVolumesFinal=500,passoVolume=100, t_inicial=0,t_final= 5,velocidade=1,alpha= 100*10**(-2),ca=0.8, cb=0.1, cc=1.0)
# Alphas(comprimento=50,  numeroVolumes=500, t_inicial=0, t_final=5, velocidade=1, alpha=100*10**(-2),ca=0.8, cb=0.1, cc=1.0)
# SensibilidadeVelocidade(comprimento = 50,numeroVolumes= 500,t_inicial=0,t_final=5,velocidadeInicial=1,velocidadeFinal=5, alpha=100**(-2),ca=0.8,cb= 0.1, cc=1.0, qtdComparacoes=5)
# SensibilidadeAlpha(comprimento = 50,numeroVolumes= 500,t_inicial= 0, t_final=5, velocidade=1, alphaInicial=1*10**(-2), alphaFinal= 100*10**(-2),ca=0.8,cb= 0.1,cc= 1.0, qtdComparacoes=5)
# Sensibilidadeca(comprimento=50, numeroVolumes=500, t_inicial=0, t_final=5, velocidade=1, alpha=100*10**(-2),caInicial=0.4, caFinal=0.9,  cb=0.2, cc=1.0, qtdComparacoes=5)
# Sensibilidadecb(comprimento=50, numeroVolumes=500, t_inicial=0, t_final=5, velocidade=1, alpha=100*10**(-2),ca=0.8, cbInicial = 0.1, cbFinal=0.5,  cc=1.0, qtdComparacoes=5)
# Sensibilidadecc(comprimento=50, numeroVolumes=500, t_inicial=0, t_final=5, velocidade=1, alpha=100*10**(-2),ca=0.8, cb=0.1, ccInicial=0.5, ccFinal = 1, qtdComparacoes=5 )
# SensibilidadeTempoFinal(comprimento=50, numeroVolumes=500, t_inicial=0, t_final=5, velocidade=1, alpha=100*10**(-2),ca=0.8, cb=0.1, cc=1.0, qtdComparacoes=5 )
# SensibilidadeComprimento(comprimento_inicial=30, comprimento_final=70, numeroVolumes=500, t_inicial=0, t_final=5, velocidade=1, alpha=100*10**(-2),ca=0.8, cb=0.1, cc=1.0, qtdComparacoes=5 )
